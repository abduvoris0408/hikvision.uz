...
// 	return (
// 		<section
// 			className={`relative bg-gradient-to-br ${slide.gradient} transition-all duration-700 overflow-hidden`}
// 		>
// 			<div className='container mx-auto px-4 lg:px-8 h-[120vh] lg:h-[80vh] flex items-center'>
...
// 								className='relative h-[350px] md:h-[450px] lg:h-[500px] rounded-2xl border border-white/20 shadow-2xl overflow-hidden'
// 							>
// 								<Image
// 									src={slide.image}
// 									alt={slide.title}
...
// 					{/* Content */}
// 					<div className='lg:col-span-7 space-y-6 order-2 lg:order-1 overflow-hidden'>
// 						<AnimatePresence mode='wait'>
// 							<motion.div
// 								key={slide.id}
...
export function HeroSection() {
	...
	// safe translator: agar t(key) xato bersa yoki kalit topilmasa fallback qaytaradi
	const safeT = (key: string, fallback: string) => {
		try {
			const res = t(key)
			// some setups might return the key string instead of throwing; handle that:
	...
	// Build localized slides — fallback sifatida original heroSlides matnlarini ishlatamiz
	const localizedSlides = heroSlides.map((cfg, i) => {
		const title = safeT(`hero.slides.${i}.title`, cfg.title)
		const subtitle = safeT(`hero.slides.${i}.subtitle`, cfg.subtitle)
		const cta = safeT(`hero.slides.${i}.cta`, cfg.cta)

		// features: har bir feature index uchun tarjima qidiring, topilmasa cfg.features dan oling
		const features =
			(cfg.features || []).map((feat, j) =>
				safeT(`hero.slides.${i}.features.${j}`, String(feat))
			) || cfg.features
	...
	const slide = localizedSlides[currentSlide]

	useEffect(() => {
		const timer = setInterval(
			() =>
				setCurrentSlide(
					p => (p + 1) % Math.max(1, localizedSlides.length)
				),
			4000
		)
		return () => clearInterval(timer)
