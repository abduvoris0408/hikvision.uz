...
// import { Clock, Heart, MapPin, Menu, Phone, ShoppingCart } from 'lucide-react'
...
// 		{ name: 'Aloqa', href: '/contact', ariaLabel: 'Contact us page' },
// 	]
...
// 		>
// 			<div className='relative p-2 rounded-lg transition-all duration-200 hover:bg-red-50/80 hover:backdrop-blur-sm'>
// 				<ShoppingCart
// 					className={`${
// 						isMobile ? 'h-6 w-6' : 'h-5 w-5'
// 					} text-red-500 group-hover:text-red-600 transition-all duration-200 group-hover:scale-105`}
// 				/>
// 				{cartCount > 0 && (
// 					<>
// 						<div className='absolute -top-1 -right-1 bg-red-500/20 backdrop-blur-md rounded-full min-w-[20px] h-[20px] scale-110' />
// 						<span className='absolute -top-1 -right-1 bg-gradient-to-br from-red-500 to-red-600 text-white text-xs font-bold rounded-full min-w-[20px] h-[20px] flex items-center justify-center shadow-lg shadow-red-500/25 ring-2 ring-white/20 backdrop-blur-sm transition-all duration-200 group-hover:scale-105'>
...
// 	const WishlistButton = ({ isMobile = false }) => (
// 		<Link
// 			href='/wishlist'
// 			className='relative group'
// 			aria-label={`Wishlist with ${wishlistCount} items`}
// 		>
// 			<div className='relative p-2 rounded-lg transition-all duration-200 hover:bg-pink-50/80 hover:backdrop-blur-sm'>
// 				<Heart
// 					className={`${
// 						isMobile ? 'h-6 w-6' : 'h-5 w-5'
// 					} text-pink-500 group-hover:text-pink-600 transition-all duration-200 group-hover:scale-105`}
// 				/>
// 				{wishlistCount > 0 && (
// 					<>
// 						<div className='absolute -top-1 -right-1 bg-pink-500/20 backdrop-blur-md rounded-full min-w-[20px] h-[20px] scale-110' />

// 						<span className='absolute -top-1 -right-1 bg-gradient-to-br from-pink-500 to-pink-600 text-white text-xs font-bold rounded-full min-w-[20px] h-[20px] flex items-center justify-center shadow-lg shadow-pink-500/25 ring-2 ring-white/20 backdrop-blur-sm transition-all duration-200 group-hover:scale-105'>
...
export function Header() {
	...
	// --- Move text-containing arrays after t() so we can use translations ---
	const navigation = [
		{ name: t('header.nav.home'), href: '/' },
		{
			name: t('header.nav.products'),
			href: '/products',
			ariaLabel: t('header.nav.products'),
		},
		{
			name: t('header.nav.portfolio'),
			href: '/portfolio',
		},
		{
	...
	const contactInfo = [
		{
			icon: Phone,
			text: t('header.contact.phone'),
		},
		{
			icon: MapPin,
			text: t('header.contact.address'),
		},
		{
			icon: Clock,
			text: t('header.contact.hours'),
		},
	]
	...
	const WishlistButton = ({ isMobile = false }: { isMobile?: boolean }) => (
		...
		>
							...
							{contactInfo.map(({ icon: Icon, text }, index) => (
								...
								>
									<Icon
										...
										aria-hidden='true'
									/>
									<span className='text-slate-700 group-hover:text-red-600 transition-colors duration-300'>
										{text}
									</span>
								</div>
							))}
			...
			<div className='sticky top-0 py-5 bg-gradient-to-r from-transparent to-red-600/5'>
				<div className='container mx-auto px-4'>
					...
					<div className='flex md:hidden items-center justify-between'>
						...
						<div className='flex items-center gap-2'>
							...
							<Button
								...
								className='relative text-slate-700 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all duration-300 hover:scale-105 group overflow-hidden border border-slate-200 hover:border-blue-300'
								onClick={() => setIsOpen(true)}
								aria-label={t('header.buttons.open_menu')}
							>
								<Menu className='h-6 w-6 transition-transform duration-300 group-hover:rotate-90' />
							</Button>
						</div>
					...
					<div className='md:hidden flex flex-col gap-4 mt-5'>
						<CatalogDropdown
							triggerText={t('header.catalog.trigger')}
							className='bg-red-400 text-white hover:from-red-700 hover:to-purple-700 font-medium rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02]'
						/>
						<div className='relative'>
							<SearchBar
								className='bg-slate-100/80 backdrop-blur-sm text-slate-700 placeholder-slate-500 rounded-xl border border-slate-200 hover:border-blue-300 transition-all duration-300 focus:ring-2 focus:ring-blue-500/20'
